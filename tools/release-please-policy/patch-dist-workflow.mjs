    ...
    steps:
      ...
      - id: identity
        ...
        run: |
          ...
          version="$(sed -n 's/^version = "\\([^"]*\\)"/\\1/p' Cargo.toml | head -1)"
          [ "$TAG" = "v$version" ]
          release="$(bash scripts/lookup-official-draft.sh "\${{ github.repository }}" "$TAG")"
      ...
      - name: Assemble and validate exact asset set
        shell: bash
        run: |
          bash scripts/assemble-official-candidate.sh target/distrib identity.json dist-manifest.json release-assets
